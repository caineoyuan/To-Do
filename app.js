...
// ============================================================
// Quick-Add Popup (Ctrl+Shift+X)
// ============================================================
let quickAddPopup = null;

function createQuickAddPopup(x, y) {
    removeQuickAddPopup();

    quickAddPopup = document.createElement("div");
    quickAddPopup.className = "quick-add-popup";
    quickAddPopup.innerHTML = `
        <form class="quick-add-form">
            <input type="text" class="quick-add-input" placeholder="Add a task..." autofocus />
            <button type="submit" class="quick-add-btn">+</button>
        </form>
    `;

    // Position at cursor
    quickAddPopup.style.left = `${x}px`;
    quickAddPopup.style.top = `${y}px`;
    document.body.appendChild(quickAddPopup);

    const input = quickAddPopup.querySelector(".quick-add-input");
    const form = quickAddPopup.querySelector(".quick-add-form");
    ...
    function onKeydown(e) {
        if (e.key === "Escape") removeQuickAddPopup();
    }
    function onClickOutside(e) {
        if (quickAddPopup && !quickAddPopup.contains(e.target)) removeQuickAddPopup();
    }
    document.addEventListener("keydown", onKeydown);
    setTimeout(() => document.addEventListener("click", onClickOutside), 10);

    quickAddPopup._cleanup = () => {
        document.removeEventListener("keydown", onKeydown);
        document.removeEventListener("click", onClickOutside);
    };
}

function removeQuickAddPopup() {
    if (quickAddPopup) {
        quickAddPopup._cleanup?.();
        quickAddPopup.remove();
        quickAddPopup = null;
    }
}
...
document.addEventListener("keydown", (e) => {
    if (e.ctrlKey && e.shiftKey && e.key === "X") {
        e.preventDefault();
        createQuickAddPopup(lastMouseX, lastMouseY);
    }
});
